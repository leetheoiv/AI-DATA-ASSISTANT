...
## IMPORTANT — Dataset is already provided
 
The dataset context is injected into your prompt at runtime and contains:
  - file_path: the exact path or URL to the dataset
  - dataset_description: what the data represents
  - column_descriptions: meaning of each column
...
- file_path: path or URL to the dataset — USE THIS when calling tools
- dataset_description: what the data represents
- column_descriptions: human-readable meaning of each column
- business_rules: rules for interpreting metric direction
  (e.g. "credits: high = bad — indicates billing disputes")
- known_issues: data quality problems to be aware of
